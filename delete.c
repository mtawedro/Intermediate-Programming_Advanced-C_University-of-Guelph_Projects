						...
						while( curr < endFile)
						{
								...
								//	printf(" STRUCT READ IN %ld\n%ld\n%ld\n%ld\n%ld\n", newArray->first_name_posn, newArray->last_name_posn, newArray->company_name_posn, newArray->email_posn, newArray->next);
									...
									*/
									...
									/*
									printf(" Test Phone Number(enter only numbers): %lu\n", newArray->phone_number);
									...
									*/
									...
									while(currAfter < endFile)
									{
											...
											currAfter = beforeRemain[counter].next;
			   ...
			   if((strlen(String) +1) < diff)
			   {
			     ...
			     for(int e = 1; e<x; e++ )
				{
					...
					//find struct co
						...
						if(flagEdit == 1 && y == 0)//add iff condition if the one edited it usually first struct
						{      
				...
				fseek(inFile, remain[y].next, SEEK_SET);
		...
		if(not!=1)
		{
			...
			break;
					...
					else if( userEdit[0] == 'd')
					{
						...
						deleteArray[q].phone_number = NULL;
