...
//find struct co
	...
	if(userInput[0] == '2')
	{
        ...
        long curr = ftell(inFile);
            ...
            if(newArray->phone_number == posArray[1].phone_number)
	        {
	...
	if(userEdit[0] == 'e')
	{
		...
		/************read in the structs after the edited struct *********/
			  ...
			  /****************** reads in information again they selected -> DIDNT INCREMENT COUNT YET! ******************/
				...
				var3= count-1;
		     */
		     /************ reads in edited array to know diff of string edited**************/
		      fseek(inFile, 45, SEEK_SET);
      ...
      // printf("%ld\n%ld\n%ld\n%ld\n%ld\n", newArray->first_name_posn, newArray->last_name_posn, newArray->company_name_posn, newArray->email_posn, newArray->next);
			...
			/***************** reads in structs from what was selected tiill end of file into array of structs *********************/
		   ...
		   while(currAfter < endFile)
		   {
			  ...
			  fread (&remain[x], sizeof( contact), 1, inFile ); 
			  
			  long temp1 = remain[x].next;
				
			  printf("Struct Pos, \n%ld\n%ld\n%ld\n%ld\n%ld\n", remain[x].first_name_posn, remain[x].last_name_posn, remain[x].company_name_posn, remain[x].email_posn, remain[x].next  );
        ...
        //fwrite(newArray, sizeof(contact), 1, inFile);
		        ...
		        fseek(inFile, 45/*nextArray[count-1]*/, SEEK_SET);
       ...
       //error trap
        ...
        if(Compname[0]!= '\0')
        {
           ...
           remain[0]. email_posn  = remain[0].company_name_posn + (strlen(Compname) +1 );
        }
        ...
        else if(Compname[0]== '\0' && Lastname[0] != '\0' )
        {
			...
			remain[0].email_posn  = remain[0].last_name_posn + (strlen(Lastname) +1 );

        }

 
        remain[0]. next = remain[0].email_posn  + (strlen(String) +1 );
   
		printf("confirm new pos %ld\n%ld\n%ld\n%ld\n%ld\n", remain[0].first_name_posn, remain[0].last_name_posn, remain[0].company_name_posn, remain[0].email_posn, remain[0].next);
			   ...
			   if((strlen(String) +1) < diff)
			   {
			     ...
			     for(int i = 1; i<x; i++ )
				{
					...
					remain[i].first_name_posn  = remain[i].first_name_posn + diff ;

					remain[i].last_name_posn = remain[i].last_name_posn + diff ;

					remain[i].company_name_posn = remain[i].company_name_posn + diff ;

					remain[i].email_posn = remain[i].email_posn + diff;

					remain[i].next  = remain[i].next + diff;
			   ...
			   for(int j = 0; j<x; j++)
			   {
				...
				fwrite(&remain[j], sizeof(contact), 1, inFile);
					
				//seeks to first name position //
                
     
				fseek(inFile, remain[j].next, SEEK_SET);
	      ...
	      for(int j = 0; j<x ; j++)
		 {
	            fseek(inFile, remain[j].first_name_posn, SEEK_SET);
   	
			    if(strlen(arrayString[at].first_name) !=0)
				{
					...
					fwrite(arrayString[at].first_name, sizeof(char),(strlen(arrayString[at].first_name) +1), inFile);
                ...
                fseek(inFile, remain[j].last_name_posn, SEEK_SET);
	     	
                if(strlen(arrayString[at].last_name) != 0)
		        {
			       
			       
					printf("old last name %s\n", arrayString[at].last_name);
			       
			        fwrite(arrayString[at].last_name, sizeof(char),(strlen(arrayString[at].last_name) +1), inFile);
		       ...
		       fseek(inFile, remain[j].company_name_posn, SEEK_SET);

			   if(strlen(arrayString[at].company_name) != 0)
			   {
				  
				  printf("old company %s\n", arrayString[at].company_name);
			       
				  fwrite(arrayString[at].company_name, sizeof(char),(strlen(arrayString[at].company_name) +1), inFile);
				...
				if(flagEdit == 1 && j == 0)//add iff condition if the one edited it usually first struct
				{      
					printf("updated email %s\n", String);
					
					fseek(inFile, remain[j].email_posn, SEEK_SET);
			   ...
			   else
			   {
				   ...
				   fwrite(arrayString[at].email, sizeof(char),(strlen(arrayString[at].email) +1), inFile);
        ...
        else
	    {
		    ...
		    at++;
		     ...
		     printf("COUNT AFTER INCREMENTED %d", count);
